...
# Я так и не понял как работает execute_batch. 12 часов бился так и не решил проблему как заставить записать в базу
# через try: info('Write to PostgreSQL table {}'.format(table_name)) for i in data_dict: column = i.keys() values = [
# i[column] for column in column]
#
#         execute_batch(cursor, sql, "".join(map(str, valuess[0:-1])))
#         # cursor.execute(sql, values)
